...
## 架构依赖
```
adapter  →  facade  →  application  →  domain  ←  infrastructure
   ↓           ↓            ↓             ↑            ↑
  (只能由外指向内，禁止反向依赖)
```
- ✅ `application` 可依赖 `domain`
- ✅ `infrastructure` 可依赖 `domain`（实现其接口）
- ✅ `adapter` 可依赖 `facade` / `application`
- ❌ `domain` **不得**依赖 `application` / `infrastructure` / `facade` / `adapter`
- ❌ `facade` **不得**包含业务实现，只能定义契约
- ❌ `infrastructure` 不得将外部 POJO 直接传入 `domain`
...
- **Command**（写操作）：`XxxCommandFacade` + `XxxCommandFacadeImpl`，负责状态变更。
- **Query**（读操作）：`XxxQueryFacade` + `XxxQueryFacadeImpl`，负责数据查询。
- 命令与查询在 facade 与 application 中**严格分文件、分接口**，不得混写。

## 编码自检

- [ ] 文件是否放在了正确的分层目录与包路径下？
- [ ] 类名后缀是否符合本规范（Facade / Impl / Assembler / Converter / Validator / Repository / Factory / DTO / Enum / Event）？
- [ ] 是否存在 domain 反向依赖外层的 import？
- [ ] Command 与 Query 是否分离？
- [ ] infrastructure 是否通过实现 domain 接口而非直接调用 domain 模型？
- [ ] 新增包/类是否破坏了依赖方向？