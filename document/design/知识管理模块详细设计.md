...
* [知识管理模块 — 详细设计](#知识管理模块--详细设计)
  ...
  * [3. 领域模型设计](#3-领域模型设计)
    ...
    * [3.5 枚举设计](#35-枚举设计)
    * [3.6 仓储接口](#36-仓储接口)
      * [3.6.1 DocumentRepository](#361-documentrepository)
      * [3.6.2 DocumentVersionRepository](#362-documentversionrepository)
      * [3.6.3 DirectoryRepository](#363-directoryrepository)
      * [3.6.4 KnowledgeChunkRepository 扩展](#364-knowledgechunkrepository-扩展)
      * [3.6.5 FileStorage](#365-filestorage)
    * [3.7 领域服务](#37-领域服务)
      * [3.7.1 DocumentDomainService](#371-documentdomainservice)
      * [3.7.2 DirectoryDomainService](#372-directorydomainservice)
  ...
  * [4. Facade 接口设计](#4-facade-接口设计)
    ...
    * [4.5 Request 对象设计](#45-request-对象设计)
      ...
      * [4.5.2 DocumentUpdateRequest](#452-documentupdaterequest)
      * [4.5.3 DocumentListQueryRequest](#453-documentlistqueryrequest)
    * [4.6 DTO / Result 对象设计](#46-dto--result-对象设计)
      * [4.6.1 DocumentDTO](#461-documentdto)
      * [4.6.2 DocumentVersionDTO](#462-documentversiondto)
      * [4.6.3 DirectoryDTO](#463-directorydto)
      * [4.6.4 DocumentListResult](#464-documentlistresult)
  * [5. 应用层设计](#5-应用层设计)
...
├── api/
│   ├── DocumentCommandFacade.java        ← upload, update, delete, restore, rollback
│   ├── DocumentQueryFacade.java          ← detail, list, versions, status
│   ├── DirectoryCommandFacade.java       ← create, update, delete
│   └── DirectoryQueryFacade.java         ← list, tree
├── dto/
│   ├── DocumentDTO.java
...
│   └── DirectoryListQueryRequest.java
├── result/
│   ├── DocumentUploadResult.java
│   └── EtlStatusResult.java
└── enums/
    ├── DocumentFormatEnum.java           ← facade 层暴露的格式枚举
    ├── DocumentStatusEnum.java           ← facade 层暴露的状态枚举
    ├── VisibilityEnum.java               ← facade 层暴露的可见范围枚举
    └── EtlStatusEnum.java                ← facade 层暴露的 ETL 状态枚举
...
> **注意：版本记录不可修改（不可变）。** 创建后不做 update、不做逻辑删除——版本历史是审计记录，必须保持完整性。回滚操作是创建一个**新版本**（内容与指定历史版本相同），而非修改已有版本。
...
public interface DocumentRepository {
    ...
    Document findByDocumentId(String userId, List<String> userDepts, String documentId);

    /** 分页查询文档列表（用户可见范围内） */
    List<Document> findByUserId(String userId, List<String> userDepts, Pagination pagination,
                                 String directoryId, DocumentStatus status, String keyword);

    /** 统计文档数（用户可见范围内） */
    int countByUserId(String userId, List<String> userDepts,
                       String directoryId, DocumentStatus status, String keyword);
...
| 方法 | 设计理由 |
|------|---------|
| `findByDocumentId(userId, userDepts, documentId)` | **接口层强制权限校验。** 不从 Session/ThreadLocal 隐式获取用户信息，编译期保证不会漏掉权限过滤 |
| `findByUserId(...)` | **多维度筛选。** directoryId 过滤目录、status 过滤状态、keyword 做标题/标签模糊搜索。参数多时不拆成 N 个方法——方法语义一致（分页查询可见文档列表） |
| `userDepts` | **部门列表而非单个部门。** 用户可能属于多个部门（如兼任），权限校验需包含所有所属部门 |
| `countByDirectoryId(directoryId)` | **删除目录前的安全校验。** 目录下有文档时禁止删除（或提示用户先移动文档） |

#### 3.6.2 DocumentVersionRepository
...
#### 3.6.4 KnowledgeChunkRepository 扩展
...
public interface KnowledgeChunkRepository {
     ...
     * @param userDepts  当前用户所属部门列表
     * @return 用户可见的 chunk 列表
     */
    List<KnowledgeChunk> searchWithPermission(String query, int topK,
                                               String userId, List<String> userDepts);
...
| 方法 | 设计理由 |
|------|---------|
| `searchWithPermission(...)` | **ES 端权限过滤。** 在 ES 查询中追加 `should` 子句：`permission_type=PUBLIC OR permission_depts IN (userDepts) OR permission_users = userId`。在 ES 层过滤而非应用层过滤，保证召回数量不因权限过滤而低于 topK |
...
public interface DocumentDomainService {
     ...
     */
    String createDocument(String userId, String title, DocumentFormat format,
     ...
     */
    void updateMetadata(String userId, String documentId, String title,
                        String summary, String directoryId, List<String> tags,
                        Visibility visibility, List<String> visibleTo);

    /**
     * 更新文档文件内容——创建新版本 + 重置 ETL 状态为 PENDING。
     * 返回新的 DocumentVersion 供调用方获取 versionId。
     */
    DocumentVersion updateContent(String userId, String documentId,
                                   String filePath, Long fileSize, String changeLog);
     ...
     */
    DocumentVersion rollback(String userId, String documentId, String targetVersionId);

    /**
     * 更新 ETL 状态。
     */
    void updateEtlStatus(String documentId, EtlStatus status, String errorMessage);
...
#### 3.7.2 DirectoryDomainService

```java
public interface DirectoryDomainService {

    /** 创建目录 */
    String createDirectory(String userId, String parentId, String name);
...
Facade 层定义知识管理模块对外暴露的 API 契约，与实现（application 层）解耦。遵循已有 CQRS 约定——读写分离。
...
public interface DocumentCommandFacade {
    ...
    /**
     * 上传文档。
     * 用户通过 multipart/form-data 上传文件，Controller 处理后调用此方法。
     */
     ...
     * 不产生新版本，不触发 ETL。
     */
    RespData<Void> updateMetadata(DocumentUpdateRequest request);

    /**
     * 更新文档文件内容（替换文件）。
     * 产生新版本，触发 ETL 重新处理。
     */
    RespData<Void> updateContent(DocumentUpdateRequest request);

    /**
     * 删除文档（移入回收站）。
     */
    RespData<Void> delete(String userId, String documentId);

    /**
     * 从回收站恢复文档。
     */
    RespData<Void> restore(String userId, String documentId);

    /**
     * 回滚到指定历史版本。
     * 创建新版本（内容与目标版本相同），触发 ETL。
     */
    RespData<Void> rollback(String userId, String documentId, String targetVersionId);
}
```
...
public interface DocumentQueryFacade {
    ...
    /** 查询文档详情（含版本列表） */
    RespData<DocumentDTO> getDocument(String userId, String documentId);

    /** 分页查询文档列表 */
    RespData<PageData<DocumentDTO>> listDocuments(DocumentListQueryRequest request);

    /** 查询文档版本历史 */
    RespData<List<DocumentVersionDTO>> listVersions(String userId, String documentId);

    /** 查询 ETL 处理状态 */
    RespData<EtlStatusResult> getEtlStatus(String userId, String documentId);
}
```
...
public interface DirectoryCommandFacade {
    ...
    /** 创建目录 */
    RespData<String> create(DirectoryCreateRequest request);

    /** 重命名 */
    RespData<Void> rename(String userId, String directoryId, String newName);

    /** 移动目录 */
    RespData<Void> move(String userId, String directoryId, String newParentId);

    /** 删除目录 */
    RespData<Void> delete(String userId, String directoryId);
}
```
...
public interface DirectoryQueryFacade {
    ...
    /** 查询子目录列表 */
    RespData<List<DirectoryDTO>> listChildren(String parentId);
...
public class DocumentUploadRequest implements Serializable {
    @NotBlank
    private String userId;                   // ① 操作主体

    private String title;                    // ② 文档标题（未填则取文件名）

    private String directoryId;              // ③ 所属目录（可空）

    private List<String> tags;               // ④ 手动标签

    @NotNull
    private Visibility visibility;           // ⑤ 可见范围

    private List<String> visibleTo;          // ⑥ 可见对象（DEPARTMENT/SPECIFIC 时必填）
...
| 字段 | 约束 | 定义原因 |
|------|------|---------|
| ① `userId` | `@NotBlank` | 操作主体标识（ADR-002） |
| ② `title` | 可空 | 未指定时从文件名提取 |
| ③ `directoryId` | 可空 | 根目录上传 |
| ④ `tags` | 可空 | ETL 后 LLM 自动补充，用户也可手动预填 |
| ⑤ `visibility` | `@NotNull` | 必须显式选择可见范围，避免"未设置导致全员可见"的安全风险 |
| ⑥ `visibleTo` | 条件必填 | visibility=DEPARTMENT/SPECIFIC 时必填，否则校验不通过 |
| ⑦ `file` | `@NotNull` | 上传文件。Controller 层完成存储后，将 filePath + fileSize 传入 application 层 |

#### 4.5.2 DocumentUpdateRequest
...
public class DocumentUpdateRequest implements Serializable {
    ...
    private String title;                    // 标题（可只更新标题不更新文件）
    private String summary;                  // 摘要（手动修改用）
    private String directoryId;              // 目录
    private List<String> tags;               // 标签
    private Visibility visibility;           // 可见范围
    private List<String> visibleTo;          // 可见对象
    private String changeLog;                // 变更说明

    private MultipartFile file;              // 新文件（可空：仅元数据变更；非空：内容变更 → 产生新版本）
}
```

#### 4.5.3 DocumentListQueryRequest

```java
public class DocumentListQueryRequest implements Serializable {
    @NotBlank
    private String userId;
...
public class DocumentDTO implements Serializable {
    ...
    private String directoryName;            // 目录名称（JOIN 查询填充）
    private List<String> tags;
    private VisibilityEnum visibility;
    private DocumentStatusEnum status;
    private EtlStatusEnum etlStatus;
    private String etlErrorMessage;          // 仅 etlStatus=FAILED 时有值
...
| 设计要点 | 说明 |
|---------|------|
| `directoryName` | **冗余展示字段。** 列表/详情展示目录名称时不需要前端二次查询 |
| `etlErrorMessage` | **条件返回。** `@JsonInclude(NON_NULL)`，仅 FAILED 时返回 |
| `content` 不在此 DTO | **文档正文通过独立的文件下载接口获取**，不在详情 DTO 中携带（可能数 MB 文本） |

#### 4.6.2 DocumentVersionDTO
...
| 设计要点 | 说明 |
|---------|------|
| 不暴露 `filePath` | **内部存储路径不对外暴露。** 文件通过独立的下载接口获取（`GET /documents/{id}/versions/{versionId}/download`） |
| 不暴露版本内容 diff | **diff 展示由前端实现。** 后端提供两个版本的纯文本供前端做 `diff_match_patch` 对比。MVPP 阶段 diff 为文本级对比 |
...
public class DirectoryDTO implements Serializable {
    ...
    private List<DirectoryDTO> children;     // 树结构子节点（仅 tree 接口填充）
    private Integer documentCount;           // 该目录下的文档数（可选）
    private LocalDateTime createTime;
}
```

#### 4.6.4 DocumentListResult

```java
public class DocumentUploadResult implements Serializable {
    private String documentId;               // 新建文档 ID
    private EtlStatusEnum etlStatus;         // 初始状态 PENDING
}
```
...
public class DocumentCommandFacadeImpl implements DocumentCommandFacade {
    ...
    public RespData<DocumentUploadResult> upload(DocumentUploadRequest request) {
        ...
        // 2. 保存文件到 FileStorage
        String filePath = buildFilePath(request.getUserId(), request.getTitle(),
                                         request.getFile().getOriginalFilename());
        long fileSize = fileStorage.store(filePath, request.getFile().getInputStream());

        // 3. 确定标题
        String title = StringUtils.isNotBlank(request.getTitle())
            ? request.getTitle()
            : extractFileNameWithoutExtension(request.getFile().getOriginalFilename());

        // 4. 创建 Document 聚合
        String documentId = documentService.createDocument(
            request.getUserId(), title, detectFormat(request.getFile()),
            fileSize, filePath, request.getDirectoryId(),
            request.getTags(), request.getVisibility(), request.getVisibleTo()
        );
    ...
    @Override
    public RespData<Void> updateMetadata(DocumentUpdateRequest request) {
        documentService.updateMetadata(
            ...
            request.getVisibility(), request.getVisibleTo()
        );
        return RespData.success();
    }

    @Override
    public RespData<Void> updateContent(DocumentUpdateRequest request) {
        // 1. 保存新文件
        ...
        // 2. 创建新版本
        DocumentVersion version = documentService.updateContent(
            request.getUserId(), request.getDocumentId(),
            filePath, fileSize, request.getChangeLog()
        );

        // 3. 删除旧 chunk + 异步触发 ETL
    ...
    @Override
    public RespData<Void> delete(String userId, String documentId) {
        documentService.delete(userId, documentId);
        // 异步将 ES chunk 标记 deleted=true
        etlOrchestrator.markDeleted(documentId);
        return RespData.success();
    }

    @Override
    public RespData<Void> restore(String userId, String documentId) {
        documentService.restore(userId, documentId);
        // 异步将 ES chunk 恢复 deleted=false
        etlOrchestrator.markRestored(documentId);
        return RespData.success();
    }

    @Override
    public RespData<Void> rollback(String userId, String documentId, String targetVersionId) {
        // 1. 获取目标版本的文件路径
        DocumentVersion targetVersion = versionRepository.findByVersionId(targetVersionId);

        // 2. 领域服务中创建新版本（复用目标版本文件）
        DocumentVersion newVersion = documentService.rollback(userId, documentId, targetVersionId);

        // 3. 触发 ETL 重新处理
        etlOrchestrator.reprocess(documentId);
...
public class DocumentQueryFacadeImpl implements DocumentQueryFacade {
    ...
    @Override
    public RespData<DocumentDTO> getDocument(String userId, String documentId) {
        // 权限上下文：从当前请求的用户信息中获取
        List<String> userDepts = getUserDeptsFromContext();

        Document document = documentRepository.findByDocumentId(userId, userDepts, documentId);
        if (document == null) {
            throw ExceptionFactory.businessException("文档不存在或无权访问");
        }
        return RespData.success(DocumentConverter.toDocumentDTO(document));
    }
    ...
    public RespData<PageData<DocumentDTO>> listDocuments(DocumentListQueryRequest request) {
        ...
        List<Document> documents = documentRepository.findByUserId(
            request.getUserId(), userDepts, request.toPagination(),
            request.getDirectoryId(), request.getStatus(), request.getKeyword()
        );
        int total = documentRepository.countByUserId(
        ...
        return RespData.success(PageData.of(dtos, total, request.getPageNum(), request.getPageSize()));
    }
}
```
...
public class DocumentEtlOrchestrator {
    ...
    public void process(String documentId) {
        try {
            ...
            // Phase 1: 解析
            documentService.updateEtlStatus(documentId, EtlStatus.PARSING, null);
            DocumentParser parser = parserRegistry.getParser(document.getFormat());
            ParsedDocument parsed = parser.parse(document.getFilePath());

            // Phase 2: 分块
            documentService.updateEtlStatus(documentId, EtlStatus.CHUNKING, null);
            List<KnowledgeChunk> chunks = chunker.chunk(parsed, document);

            // Phase 3: 增强（LLM 摘要/标签）
            documentService.updateEtlStatus(documentId, EtlStatus.ENRICHING, null);
            ...
            // Phase 4: 向量化
            documentService.updateEtlStatus(documentId, EtlStatus.EMBEDDING, null);
            embedChunks(chunks);

            // Phase 5: 索引
            documentService.updateEtlStatus(documentId, EtlStatus.INDEXING, null);
            chunkRepository.index(chunks);
            ...
            documentService.updateEtlStatus(documentId, EtlStatus.FAILED,
...
public class DocumentCommandController {
    ...
    private final DocumentCommandFacade commandFacade;
    private final FileStorage fileStorage;
    ...
    public RespData<DocumentUploadResult> upload(
        ...
        return commandFacade.upload(request);
    }
    ...
    public RespData<Void> updateMetadata(
            @PathVariable String documentId,
            @Valid @RequestBody DocumentUpdateRequest request,
            HttpServletRequest httpRequest) {
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        return commandFacade.updateMetadata(request);
    }

    /**
     * 更新文档内容（替换文件）—— multipart/form-data
     */
    @PostMapping(value = "/{documentId}/content", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public RespData<Void> updateContent(
            @PathVariable String documentId,
            @Valid @ModelAttribute DocumentUpdateRequest request,
            HttpServletRequest httpRequest) {
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        return commandFacade.updateContent(request);
    }
    ...
    public RespData<Void> delete(
        ...
        return commandFacade.delete(userId, documentId);
    }

    /** 恢复文档 */
    @PostMapping("/{documentId}/restore")
    public RespData<Void> restore(
            @PathVariable String documentId,
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return commandFacade.restore(userId, documentId);
    }

    /** 回滚到指定版本 */
    @PostMapping("/{documentId}/versions/{versionId}/rollback")
    public RespData<Void> rollback(
            @PathVariable String documentId,
            @PathVariable String versionId,
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return commandFacade.rollback(userId, documentId, versionId);
    }
...
public class DocumentQueryController {
    ...
    private final DocumentQueryFacade queryFacade;

    /** 文档详情 */
    @GetMapping("/{documentId}")
    public RespData<DocumentDTO> getDocument(
            @PathVariable String documentId,
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return queryFacade.getDocument(userId, documentId);
    }

    /** 文档列表 */
    @GetMapping
    public RespData<PageData<DocumentDTO>> listDocuments(
            @Valid DocumentListQueryRequest request,
            HttpServletRequest httpRequest) {
        request.setUserId(extractUserId(httpRequest));
        return queryFacade.listDocuments(request);
    }

    /** 版本历史 */
    @GetMapping("/{documentId}/versions")
    public RespData<List<DocumentVersionDTO>> listVersions(
            @PathVariable String documentId,
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return queryFacade.listVersions(userId, documentId);
    }

    /** ETL 状态 */
    @GetMapping("/{documentId}/etl-status")
    public RespData<EtlStatusResult> getEtlStatus(
            @PathVariable String documentId,
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return queryFacade.getEtlStatus(userId, documentId);
    }
}
```
...
public class DirectoryCommandController {
    ...
    private final DirectoryCommandFacade commandFacade;

    @PostMapping
    public RespData<String> create(
            @Valid @RequestBody DirectoryCreateRequest request,
            HttpServletRequest httpRequest) {
        request.setUserId(extractUserId(httpRequest));
        return commandFacade.create(request);
    }

    @PostMapping("/{directoryId}/rename")
    public RespData<Void> rename(
            @PathVariable String directoryId,
            @RequestParam String name,
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return commandFacade.rename(userId, directoryId, name);
    }

    @PostMapping("/{directoryId}/move")
    public RespData<Void> move(
            @PathVariable String directoryId,
            @RequestParam String newParentId,
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return commandFacade.move(userId, directoryId, newParentId);
    }
    ...
    public RespData<Void> delete(
            ...
            HttpServletRequest httpRequest) {
        String userId = extractUserId(httpRequest);
        return commandFacade.delete(userId, directoryId);
    }
}
```
...
public class DirectoryQueryController {
    ...
    private final DirectoryQueryFacade queryFacade;

    /** 子目录列表 */
    @GetMapping("/children")
    public RespData<List<DirectoryDTO>> listChildren(@RequestParam(required = false) String parentId) {
        return queryFacade.listChildren(parentId);
    }

    /** 完整目录树 */
    @GetMapping("/tree")
    public RespData<List<DirectoryDTO>> tree() {
        return queryFacade.tree();
    }
}
```
...
上传后前端轮询 `GET /api/v1/documents/{documentId}/etl-status` 直到 `etlStatus` 变为 `completed` 或 `failed`。
...
== 删除 ==
User -> Controller: POST /{documentId}/delete
Controller -> Facade: delete(userId, documentId)
...
== 恢复 (30天内) ==
User -> Controller: POST /{documentId}/restore
Controller -> Facade: restore(userId, documentId)
...
User -> Controller: POST /{documentId}/versions/{versionId}/rollback

Controller -> Facade: rollback(userId, documentId, versionId)

Facade -> VerRepo: findByVersionId(versionId)
...
```
POST /api/v1/documents/{documentId}/metadata
...
```
GET /api/v1/documents?directoryId=dir_xxx&status=published&keyword=产品&pageNum=1&pageSize=20
X-User-Id: user_12345
...
```
POST /api/v1/documents/{documentId}/delete
X-User-Id: user_12345
...
{"parentId": null, "name": "产品文档"}

# 子目录列表
GET /api/v1/directories/children?parentId=dir_xxx

# 完整目录树
GET /api/v1/directories/tree

# 重命名
POST /api/v1/directories/{directoryId}/rename?name=新名称

# 移动
POST /api/v1/directories/{directoryId}/move?newParentId=dir_new
...
```
GET /api/v1/documents/{documentId}/etl-status
X-User-Id: user_12345
...
| 设计决策 | 理由 |
|---------|------|
| **无 `update_time` / `deleted`** | 版本记录不可变——创建后不修改、不删除 |
| **无 `updater`** | 同上，没有更新操作 |
