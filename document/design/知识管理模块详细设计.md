...
public class DocumentUploadRequest extends BaseRequest implements Serializable {
    ...
    // MultipartFile 由 Controller 层处理，此处存 filePath + fileSize
    private String filePath;                 // Controller 完成存储后注入
    private Long fileSize;                   // Controller 完成存储后注入
    private DocumentFormatEnum format;       // Controller 根据文件扩展名注入
}
```
...
public class DocumentCommandFacadeImpl implements DocumentCommandFacade {
    ...
    public RespData<DocumentUploadResult> upload(DocumentUploadRequest request) {
        // Controller 已完成文件存储、格式检测，此处直接构造领域命令
        DocumentUploadValidator.validate(request);

        String documentId = documentService.createDocument(new CreateDocumentCommand(
            request.getUserId(), request.getTitle(), DocumentFormat.valueOf(request.getFormat().getCode()),
            request.getFileSize(), request.getFilePath(), request.getDirectoryId(),
            request.getTags(), Visibility.valueOf(request.getVisibility().getCode()),
    ...
    public RespData<DocumentUpdateMetadataResult> updateMetadata(DocumentUpdateMetadataRequest request) {
        documentService.updateMetadata(new UpdateDocumentMetadataCommand(
            ...
            Visibility.valueOf(request.getVisibility().getCode()), request.getVisibleTo()
        ));
    ...
    public RespData<DocumentUpdateContentResult> updateContent(DocumentUpdateContentRequest request) {
        // Controller 已完成文件存储，此处直接创建新版本
        documentService.updateContent(new UpdateDocumentContentCommand(
            request.getUserId(), request.getDocumentId(),
            request.getFilePath(), request.getFileSize(), request.getChangeLog()
        ));
...
public class DocumentQueryFacadeImpl implements DocumentQueryFacade {
    ...
    public RespData<EtlStatusResult> getEtlStatus(EtlStatusQueryRequest request) {
        ...
        result.setEtlStatus(EtlStatusEnum.valueOf(document.getEtlStatus().getCode()));
        result.setErrorMessage(document.getEtlErrorMessage());
        result.setDocumentStatus(DocumentStatusEnum.valueOf(document.getStatus().getCode()));
        return RespData.success(result);
    }
}
```
...
public class DocumentCommandController {
    ...
    public RespData<DocumentUploadResult> upload(
            ...
            @RequestParam(required = false) List<String> visibleTo,
            HttpServletRequest httpRequest) throws IOException {

        // 保存文件
        String filePath = buildFilePath(extractUserId(httpRequest), title,
                                         file.getOriginalFilename());
        long fileSize = fileStorage.store(filePath, file.getInputStream());
    ...
    public RespData<DocumentUpdateMetadataResult> updateMetadata(
            ...
            @Valid @RequestBody DocumentUpdateMetadataRequest request,
    ...
    public RespData<DocumentUpdateContentResult> updateContent(
        ...
        String filePath = buildVersionFilePath(userId, documentId,
                                                file.getOriginalFilename());
        long fileSize = fileStorage.store(filePath, file.getInputStream());
    ...
    public RespData<DocumentDeleteResult> delete(
        ...
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        return commandFacade.delete(request);
    }
    ...
    public RespData<DocumentRestoreResult> restore(
        ...
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        return commandFacade.restore(request);
    }
    ...
    public RespData<DocumentRollbackResult> rollback(
            ...
            HttpServletRequest httpRequest) {
        DocumentRollbackRequest request = new DocumentRollbackRequest();
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        request.setTargetVersionId(versionId);
        return commandFacade.rollback(request);
    }
...
public class DocumentQueryController {
    ...
    public RespData<DocumentDetailResult> getDocument(
            @PathVariable String documentId,
            HttpServletRequest httpRequest) {
        DocumentDetailQueryRequest request = new DocumentDetailQueryRequest();
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        return queryFacade.getDocument(request);
    }
    ...
    public RespData<DocumentVersionListResult> listVersions(
            @PathVariable String documentId,
            HttpServletRequest httpRequest) {
        DocumentVersionListRequest request = new DocumentVersionListRequest();
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        return queryFacade.listVersions(request);
    }
    ...
    public RespData<EtlStatusResult> getEtlStatus(
            @PathVariable String documentId,
            HttpServletRequest httpRequest) {
        EtlStatusQueryRequest request = new EtlStatusQueryRequest();
        request.setUserId(extractUserId(httpRequest));
        request.setDocumentId(documentId);
        return queryFacade.getEtlStatus(request);
    }
}
```
...
public class DirectoryCommandController {
    ...
    public RespData<DirectoryRenameResult> rename(
            ...
            HttpServletRequest httpRequest) {
        DirectoryRenameRequest request = new DirectoryRenameRequest();
        request.setUserId(extractUserId(httpRequest));
        request.setDirectoryId(directoryId);
        request.setNewName(newName);
        return commandFacade.rename(request);
    }
    ...
    public RespData<DirectoryMoveResult> move(
            ...
            HttpServletRequest httpRequest) {
        DirectoryMoveRequest request = new DirectoryMoveRequest();
        request.setUserId(extractUserId(httpRequest));
        request.setDirectoryId(directoryId);
        request.setNewParentId(newParentId);
        return commandFacade.move(request);
    }
...
public class DirectoryQueryController {
    ...
    public RespData<List<DirectoryDTO>> listChildren(
        ...
        DirectoryChildrenQueryRequest request = new DirectoryChildrenQueryRequest();
        request.setParentId(parentId);
        return queryFacade.listChildren(request);
    }
...
```
GET /api/v1/documents?directoryId=dir_xxx&status=published&keyword=产品&page=1&size=20
X-User-Id: user_12345
